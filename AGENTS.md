...
本仓库当前阶段优先实现后端微服务系统，同时保留并迭代一个轻量前端用于联调与验收。项目重点是后端服务拆分、认证鉴权、用户关系、会话管理、消息存储、实时通信，以及后续 AI Bot 和 RAG 知识库能力的扩展。
...
- P0：已完成（微服务骨架、注册登录、JWT 校验、服务间 RPC 调用）。
- P1：已完成并扩展（会话/群聊/消息持久化/历史查询/好友关系等主链路已稳定）。
- P2：已完成（WebSocket 长连接、实时消息推送、群聊广播已接入并联调）。
- P3：部分完成（会话 Bot 管理、@Bot 基础触发链路、AI 调用日志已落地，仍在持续完善）。
- P4：未开始（RAG 知识库能力暂未进入实现阶段）。
...
当前阶段包含前端联调页面（`frontend/`），用于功能验收、回归测试与交互优化；核心业务能力仍以后端服务为主。
...
- 禁止使用 PowerShell 脚本进行代码文件内容改写（包括批量替换、正则替换、重写文件）。
- 允许使用 PowerShell 仅做只读排查（查看日志、查看状态、读取信息）。
- 所有代码文件与文档文件的实际修改，统一使用 `apply_patch` 完成。
- 目的：避免转义字符、编码污染、换行符异常等由 shell 文本处理带来的问题。
