...
func (s *ChatService) UpdateGroupAvatar(ctx context.Context, input UpdateGroupAvatarInput) (*ConversationEventView, error) {
	if input.OperatorID == 0 {
		return nil, ErrBadRequest
	}
	...
	err = s.TxManager.WithinTransaction(ctx, func(tx *gorm.DB) error {
		...
		if group.Avatar == avatar && conversation.Avatar == avatar {
			return nil
		}
		...
		systemMessage, recipientUserIDs, eventErr := s.createSystemMessageTx(
			...
			messageRepo,
			s.notificationRepoWithTx(tx),
			model.SystemEventGroupAvatarUpdated,
			input.OperatorID,
			nil,
		)
		if eventErr != nil {
